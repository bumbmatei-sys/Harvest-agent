...
// NOTE: The per-donation `generateReceiptPDF` helper and its `generateSingleReceipt`
// onCreate trigger were retired — see the retirement note where function #4 used to
// live (below `backfillCRMContacts`). Per-donation receipts are now generated by the
// Next.js webhook (`issueDonationReceipt` in src/lib/donation-receipt.ts), the single
// authoritative writer. Only the ANNUAL receipt helper remains here.
...
// ─── 4. generateSingleReceipt — RETIRED ─────────────────────────────────────────────
//
// This was a Firestore onCreate trigger on tenants/{tenantId}/invoices/{invoiceId}
// that generated a SECOND per-donation receipt PDF. It has been REMOVED because it
// raced the authoritative generator:
//
//   • The Stripe webhook creates the invoice and then calls `issueDonationReceipt`
//     (src/lib/donation-receipt.ts), which generates ONE PDF at the human-readable
//     path `receipts/{tenantId}/donations/{receiptNumber}.pdf`, records it as a bare
//     `pdfUrl`, sets status 'sent'/'stored', and emails the donor.
//   • This trigger fired on EVERY invoice create (no type guard, no already-generated
//     guard), produced a duplicate PDF at `tenants/{tenantId}/invoices/{invoiceId}.pdf`,
//     and wrote `{ pdfPath, pdfUrl: null, status: 'generated' }`. When it landed last
//     it NULLED the `pdfUrl` the webhook had just written, which is exactly what the
//     member-facing download route (/api/donation-history/download) needs. That is why
//     downloads 404'd.
//
// `issueDonationReceipt` is now the SINGLE authoritative writer of a donation invoice's
// pdfUrl/status. Do NOT reintroduce an invoice-create trigger here.
//
// ⚠️ REMOVING THIS EXPORT DOES NOT UNDEPLOY THE RUNNING FUNCTION. There is no CI step
// that deploys Cloud Functions (deploy-rules.yml deploys only firestore:rules + storage).
// The already-deployed `generateSingleReceipt` keeps running — and keeps racing — until
// someone runs `firebase deploy --only functions` (Firebase detects the removed export
// and deletes the deployed function). Deploy from Cloud Shell to complete the fix.
