...
export async function GET(request: NextRequest) {
  ...
  if (!connectedAccountId) {
    return NextResponse.redirect(new URL('/admin/settings?instagram_error=missing_connection_id', tenantBaseUrl));
  }
  ...
  try {
    ...
    if (connectionStatus.status.toUpperCase() !== 'ACTIVE') {
      ...
      return NextResponse.redirect(new URL('/admin/settings?instagram_error=connection_not_active', tenantBaseUrl));
    }
