...
const NOT_OURS: Record<string, ReadonlyArray<readonly [digest: string, source: string]>> = {
  ...
  'src/components/events/ServicePlanRow.tsx': [
    ['ffafaf4228bbefe95d7b9bf439d5a36df9c9160f5c086cbae25a236ec0adca37', 'main at 0de1e2e — THE-317 owns it and has not landed'],
  ],
};

describe('the four files this ticket does not own are byte-identical', () => {
  it.each(Object.keys(NOT_OURS))('%s carries no edit from THE-319', (file) => {
  ...
  it.each(Object.keys(NOT_OURS))('%s does not mention THE-319', (file) => {
