...
const UNTOUCHED: Record<string, ReadonlyArray<readonly [digest: string, source: string]>> = {
  'src/components/AdminDashboard.tsx': [
    ...
    ['465b72f66ae55ec668f5bc7407c49edaeee0be5f5b20e028f6a64e57de6a8397',
      `main + THE-191 - the campus cap offers the add-on, so AdminChurches needs a door to the screen that sells it. ONE prop on the existing <AdminChurches /> mount: onOpenBilling, wired to the SAME expression the account menu already passes - billingAccess === 'yes' ? () => setShowBilling(true) : undefined - so an admin without billing access still gets undefined and no button. Nothing else in this file moves: no nav array, no group, no tab id, no permission gate, no render arm, no import, and no new state. setShowBilling and billingAccess both already existed for the account menu; this is a second reader of them, not a new capability.`],
  ],
