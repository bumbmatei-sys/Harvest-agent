    ...
    <div className="fixed inset-0 z-[200] bg-[#0b1121] flex flex-col">
      ...
      {showPrayer && (
        <div className="fixed inset-0 z-[210] flex items-end justify-center bg-black/50 lg:hidden" onClick={() => setShowPrayer(false)}>
          <div className="w-full max-w-lg bg-surface-raised rounded-t-2xl p-5" style={{ paddingBottom: 32 }} onClick={(e) => e.stopPropagation()}>
            {prayerDone ? (
              <div className="text-center py-6">
                <p className="text-lg font-bold text-strong font-display">🙏 Prayer received</p>
                <p className="text-sm text-muted mt-1">Our team will be praying for you.</p>
              </div>
            ) : (
              <>
                <h3 className="text-lg font-bold text-strong mb-3 font-display">Submit Prayer Request</h3>
                <input
                  value={prayerName}
                  onChange={(e) => setPrayerName(e.target.value)}
                  placeholder="Your name"
