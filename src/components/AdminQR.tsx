...
const AdminQR: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto" style={{ paddingBottom: 120 }}>
      ...
      <div className="bg-surface-raised rounded-2xl border border-line p-5 space-y-4">
        {/* Context selector */}
        {needsContext && (
          <div>
            ...
            {contextList.length === 0 && (
              <p className="text-xs text-faint mt-1.5">No {qrType === 'event' ? 'published events' : qrType === 'checkin' ? 'active sessions' : 'active forms'} yet.</p>
            )}
          </div>
        )}
        ...
        {qrType === 'giving' && (
          <p className="text-sm text-muted">This QR opens your giving page so people can donate from their phone.</p>
        )}
      ...
      {/* QR display */}
      {qrDataUrl ? (
        <div className="bg-surface-raised rounded-2xl border border-line p-6 mt-4 text-center">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={qrDataUrl} alt={`${label} QR code`} className="w-[300px] h-[300px] mx-auto" />
          {label && <p className="text-sm font-mono text-body mt-3">{label}</p>}
          <p className="text-[11px] text-faint mt-1 break-all">{truncated}</p>
          <button onClick={download} className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border border-line text-body hover:bg-surface-sunken">
            <Download size={14} /> Download PNG
          </button>
        </div>
      ) : (
        <div className="text-center py-12 text-faint mt-4">
