...
const AdminChurches: React.FC = () => {
  ...
  if (isAdding || editingChurch) {
    return (
      <div className="bg-white dark:bg-[#252a36] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Church List</h2>
        <button
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 text-sm bg-white dark:bg-[#252a36] border border-[#d4a017] rounded-full focus:outline-none text-gray-900 dark:text-white font-medium placeholder:text-gray-500 w-32 focus:w-48 transition-all duration-300"
          />
        </div>
        
        <button
          onClick={() => openFilterPopup('city')}
          className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${cityFilter ? 'bg-[#d4a017] text-white border-[#d4a017]' : 'bg-white dark:bg-[#252a36] text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-700 hover:border-[#d4a017]'}`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${pastorFilter ? 'bg-[#d4a017] text-white border-[#d4a017]' : 'bg-white dark:bg-[#252a36] text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-700 hover:border-[#d4a017]'}`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${countryFilter ? 'bg-[#d4a017] text-white border-[#d4a017]' : 'bg-white dark:bg-[#252a36] text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-700 hover:border-[#d4a017]'}`}
        >
      ...
      <div className="bg-white dark:bg-[#252a36] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-[#1a1d27] border-b border-gray-100 dark:border-gray-800">
                <th className="p-4 text-sm font-semibold text-gray-600 dark:text-gray-400">Name</th>
                <th className="p-4 text-sm font-semibold text-gray-600 dark:text-gray-400">City</th>
                <th className="p-4 text-sm font-semibold text-gray-600 dark:text-gray-400">Pastor</th>
            ...
            <tbody>
              {loading ? (
                ...
                filteredChurches.map((church) => (
                  <tr key={church.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-[#1a1d27]/50 transition-colors">
                    <td className="p-4">
                      <div className="font-medium text-gray-900 dark:text-white">{church.name}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{church.denomination}</div>
                    </td>
                    <td className="p-4 text-gray-700 dark:text-gray-300">
                      {church.city}{church.country ? `, ${church.country}` : ''}
                    </td>
                    <td className="p-4 text-gray-700 dark:text-gray-300">{church.pastorName}</td>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#252a36] rounded-2xl p-6 max-w-md w-full shadow-xl">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Delete Church</h3>
      ...
      {activeFilterPopup && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#252a36] rounded-xl shadow-xl w-full max-w-sm p-6">
            ...
            <input
              ...
              className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-[#1a1d27] text-gray-900 dark:text-white focus:outline-none focus:border-[#d4a017] mb-6"
            />
            <div className="flex justify-end gap-3">
              <button
                onClick={() => {
