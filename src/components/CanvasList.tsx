  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-strong font-display">Canvas</h2>
        <button
      ...
      {canvases.length === 0 ? (
        <div className="bg-surface-raised rounded-2xl border border-line p-12 text-center">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-[color-mix(in_srgb,var(--brand-color)_12%,transparent)] flex items-center justify-center mb-4">
            <FileText size={28} style={{ color: 'var(--brand-color, #d4a017)' }} />
          </div>
          <h3 className="text-lg font-bold text-strong mb-2 font-display">No canvases yet</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {canvases.map((canvas) => (
            <div
              ...
              className="bg-surface-raised rounded-2xl border border-line p-5 cursor-pointer hover:shadow-md hover:border-line transition-all group"
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-faint hover:text-red-500 transition-all p-1"
                >
                  <Trash2 size={16} />
                </button>
              </div>
              <h3 className="text-sm font-bold text-strong mb-1 truncate">{canvas.name}</h3>
