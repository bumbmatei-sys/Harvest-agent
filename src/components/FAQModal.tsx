...
const FAQItem: React.FC<{ 
  ...
  return (
    <div className="bg-white dark:bg-[#252a36] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden mb-4 transition-all duration-300">
      ...
      >
        <h4 className="text-sm font-bold text-gray-900 dark:text-white pr-4">{question}</h4>
        {isOpen ? (
      ...
      {isOpen && (
        <div className="px-5 pb-5 text-sm text-gray-600 dark:text-gray-300 leading-relaxed animate-in slide-in-from-top-2 duration-200">
          {answer}
        </div>
      )}
    </div>
...
const FAQModal: React.FC<FAQModalProps> = ({ isOpen, onClose }) => {
  ...
  const faqs = [
      ...
      answer: (
        <>
          ...
          <ul className="space-y-2 mb-3 pl-2">
            <li><strong className="text-gray-900 dark:text-white">Level 1:</strong> The Foundations (New Life in Christ)</li>
            <li><strong className="text-gray-900 dark:text-white">Level 2:</strong> Walking in the Spirit</li>
            <li><strong className="text-gray-900 dark:text-white">Level 3:</strong> Character & The Word</li>
            <li><strong className="text-gray-900 dark:text-white">Level 4:</strong> Commissioned to Serve</li>
          </ul>
    ...
    },
    {
      ...
      answer: (
        <>
          We welcome biblically-based churches and ministries to join our global network. You can enroll through our <strong className="text-gray-900 dark:text-white">Church Partner Portal</strong>. Once verified, your location will be visible to new converts in your immediate area, helping them find their spiritual family.
        </>
      )
    },
    {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#f8f9fa] dark:bg-[#1a1d27] animate-in slide-in-from-bottom-full duration-300 overflow-hidden">
      ...
      <div className="flex items-center px-4 py-4 bg-white dark:bg-[#1a1d27] border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white flex-1 text-center pr-8">FAQ</h2>
      </div>

      <div className="flex-1 overflow-y-auto p-4 pb-12">
        <div className="text-center mb-8 mt-4">
          <h2 className="text-2xl font-bold text-[#1a202c] dark:text-white mb-2">Frequently Asked Questions</h2>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Everything you need to know about the Harvest App.</p>
        </div>
