...
const ChurchDetailsModal: React.FC<ChurchDetailsModalProps> = ({ 
  ...
  return (
    <div className={`fixed inset-0 z-[9999] ${fullPage ? 'bg-[#f8f9fa] dark:bg-[#1a1d27]' : 'flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity'}`}>
      <div 
        className={fullPage ? 'w-full h-full overflow-y-auto flex flex-col relative animate-fade-in' : 'bg-[#f8f9fa] dark:bg-[#1a1d27] w-full sm:w-[500px] h-[90vh] sm:h-auto sm:max-h-[90vh] rounded-t-3xl sm:rounded-3xl overflow-y-auto flex flex-col relative animate-slide-up sm:animate-fade-in'}
      >
        {/* Close Button */}
        <button 
          onClick={onClose}
          ...
          <div className="flex flex-col pb-8">
            ...
            <div className="relative h-72 w-full">
              ...
              <div className="absolute bottom-12 left-0 w-full p-6 text-white">
                <h2 className="text-5xl font-serif font-bold mb-2 italic">{church.name}</h2>
            ...
            <div className="px-4 -mt-8 relative z-10 space-y-4">
              ...
              <div className="bg-white dark:bg-[#252a36] rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
                <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Service Details</h3>
                
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-[#f3f4f6] dark:bg-gray-800 flex items-center justify-center flex-shrink-0 text-[#1e293b] dark:text-white">
                      <User size={24} />
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Lead Pastor:</p>
                      <p className="font-bold text-lg text-gray-900 dark:text-white">{church.pastorName || 'Not specified'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Service Schedule:</p>
                      <div className="text-base text-gray-900 dark:text-white">{formatServices(church.services || [])}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Connect Card */}
              <div className="bg-white dark:bg-[#252a36] rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
                <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Connect</h3>
                  ...
                  {church.contactEmail && (
                    <div className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800 last:border-0">
                      <div className="flex items-center gap-4 overflow-hidden">
                        ...
                        <span className="text-base text-gray-800 dark:text-gray-200 truncate">{church.contactEmail}</span>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        <button 
                          onClick={() => handleCopy(church.contactEmail!, 'email')}
                          className="w-10 h-10 rounded-xl bg-[#f3f4f6] dark:bg-gray-800 flex items-center justify-center text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                        >
                  ...
                  {church.contactPhone && (
                    <div className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800 last:border-0">
                      <div className="flex items-center gap-4 overflow-hidden">
                        ...
                        <span className="text-base text-gray-800 dark:text-gray-200 truncate">{church.contactPhone}</span>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        <button 
              ...
              {/* Action Buttons Card */}
              <div className="bg-white dark:bg-[#252a36] rounded-3xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  {church.facebook && (
                    <a 
                  ...
                  {church.website && (
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-12 h-12 rounded-full bg-[#f1f5f9] dark:bg-gray-800 flex items-center justify-center text-[#0f172a] dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                    >
                      <Globe size={24} />
                    </a>
                  )}
